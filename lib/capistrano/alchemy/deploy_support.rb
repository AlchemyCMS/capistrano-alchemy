...
module Capistrano::Alchemy::DeploySupport
  ...
  def db_import_cmd(server)
    ...
    sql_stream = "ssh -p #{server.port} #{server.user}@#{server.hostname} '#{dump_cmd}'"
  ...
  def db_export_cmd(server)
    ...
    ssh_cmd = "ssh -p #{server.port} #{server.user}@#{server.hostname} '#{import_cmd}'"
  ...
  def send_files(type, server)
    ...
    system "rsync --progress -rue 'ssh -p #{server.port}' uploads/#{type} #{server.user}@#{server.hostname}:#{shared_path}/uploads/"
  end
  ...
  def get_files(type, server)
    ...
    system "rsync --progress -rue 'ssh -p #{server.port}' #{server.user}@#{server.hostname}:#{shared_path}/uploads/#{type} ./uploads/"
  end
end
